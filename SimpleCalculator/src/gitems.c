...
void GItemChar_fontId(GItem* _this, int id)
{
	...
	_this->_fontId = (id >= (sizeof(_fontList)/sizeof(_fontList[0]))) 
		? (sizeof(_fontList) / sizeof(_fontList[0])) - 1 : id;
}
...
int GItemChar_width(GItem* _this, HDC hdc)
{
	...
	SelectObject(hdc, _fontList[_this->_fontId]);
